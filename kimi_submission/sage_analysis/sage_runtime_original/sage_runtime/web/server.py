...
class WebServer:
    ...
    def _setup_routes(self):
        ...
        @self.app.post("/api/contracts/create")
        async def create_contract(request: Dict[str, Any]):
            """Create a new contract."""
            validator = self.kernel.get_component('contract_validator')
            if not validator:
                return {"error": "Contract validator not available"}
            ...
            try:
                ...
                logger.error(f"[WEB] Create contract error: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }

        @self.app.post("/api/contracts/{contract_id}/validate")
        async def validate_contract(contract_id: str):
            ...
            try:
                result = await validator.validate_contract(contract_id)
                ...
                logger.error(f"[WEB] Validate contract error: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }

        @self.app.post("/api/contracts/{contract_id}/approve")
        async def approve_contract(contract_id: str, request: Dict[str, Any]):
            """Approve a contract."""
            validator = self.kernel.get_component('contract_validator')
            if not validator:
                return {"error": "Contract validator not available"}
            ...
            try:
                result = await validator.approve_contract(
                    contract_id=contract_id,
                    reviewer=request.get("reviewer", "system")
                )
                ...
                logger.error(f"[WEB] Approve contract error: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }
        ...
        async def list_contracts(status: Optional[str] = None):
            ...
            try:
                ...
                contracts = validator.list_contracts(status_filter)
                ...
                logger.error(f"[WEB] List contracts error: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }

        @self.app.post("/api/rfc/create")
        async def create_rfc(request: Dict[str, Any]):
            """Create a new RFC."""
            validator = self.kernel.get_component('contract_validator')
            if not validator:
                return {"error": "Contract validator not available"}
            ...
            try:
                ...
                logger.error(f"[WEB] Create RFC error: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }

        @self.app.post("/api/rfc/{rfc_id}/submit")
        async def submit_rfc(rfc_id: str):
            ...
            try:
                result = await validator.submit_rfc(rfc_id)
                ...
                logger.error(f"[WEB] Submit RFC error: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }

        @self.app.get("/api/rfc")
        async def list_rfcs():
            """List all RFCs."""
            validator = self.kernel.get_component('contract_validator')
            if not validator:
            ...
            try:
                ...
                logger.error(f"[WEB] List RFCs error: {e}")
                return {
                    "success": False,
                    "error": str(e)
                }
