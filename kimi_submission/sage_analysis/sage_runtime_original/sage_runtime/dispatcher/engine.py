...
class TaskDispatcher:
    ...
    def __init__(self, max_concurrent: int = 3):
        ...
        self._task_counter = 0  # Tie-breaker for heap
    ...
    async def stop(self):
        ...
        while self._running_tasks:
            logger.info(f"[DISPATCHER] Waiting for {len(self._running_tasks)} running tasks...")
    ...
    async def dispatch(self, command: str, priority: TaskPriority = TaskPriority.MEDIUM) -> Task:
        ...
        heapq.heappush(self._task_queue, (-priority_value, self._task_counter, task))
        logger.info(f"[DISPATCHER] Dispatched task {task.task_id}: {command}")
        
        return task

    async def _process_tasks(self):
        """Process tasks from the queue with error recovery."""
        while self._running:
            try:
                if self._task_queue:
                    ...
                    if task.task_id not in self._running_tasks:
                        ...
                        logger.warning(f"[DISPATCHER] Task {task.task_id} already running, skipping")
                else:
                    await asyncio.sleep(0.1)
            except asyncio.CancelledError:
                logger.info("[DISPATCHER] Task processor cancelled")
                break
    ...
    async def _execute_task(self, task: Task):
        ...
        async with self._semaphore:
            try:
                ...
                logger.debug(f"[DISPATCHER] Executing task {task.task_id}")
                ...
                task.completed_at = datetime.now()
                logger.warning(f"[DISPATCHER] Task {task.task_id} cancelled")
